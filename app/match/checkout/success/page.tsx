...
export default function MatchCheckoutSuccessPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        window.fbq?.("track", "Purchase", { value: Number(amount), currency: "KRW" });
        if (!cancelled) {
          window.location.href = `/match/report?id=${orderId}`;
        }
      } catch (error) {
        if (!cancelled) {
          setState({
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-white px-6 text-center text-black">
      <Container className="flex flex-col items-center">
        ...
        <p className="mt-5 text-sm font-semibold text-black">결제를 확인하고 있어요...</p>
      </Container>
    </main>
  );
}
